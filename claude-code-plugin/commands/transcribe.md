---
description: Transcribe an audio file (OGG/MP3/M4A/WAV) to text via WhisperHeim's local speech-to-text API
...
`$ARGUMENTS` is the path to the audio file (OGG / MP3 / M4A / WAV / …). The path may be quoted and may contain spaces.
...
- **`EXIT=2`** — WhisperHeim returned an HTTP error (e.g. unsupported/corrupt audio). Show the error body the script printed to stderr.
