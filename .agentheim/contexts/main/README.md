...
- **Peak normalization** (main-hh6zw) — `AudioLevelNormalizer.PeakNormalize`, a pure static transform applied inside `TranscriptionService.DecodeAudio` (under the decode lock, after the ADR-0006 self-heal reload, before `AcceptWaveform`) to every buffer any consumer decodes. Scales quiet audio up so its peak amplitude reaches `AudioLevelNormalizer.TargetPeak` (0.5, sweep-verified against the sherpa-onnx `NemoNormalizePerFeature` float32-cancellation bug — infrastructure-anvty's diagnosis), unless the buffer is already louder (never attenuates) or its peak is below `AudioLevelNormalizer.SilenceFloor` (1e-4 — never amplifies digital silence/noise into manufactured text; matches the overlay's `NoMic` RMS threshold). The `Transcribed …` trace line reports the measured input peak and applied gain (e.g. `peak=0.012 gain=41.7x`, or `gain=1x`) so a future empty result can be correlated with input level without reproducing it.
